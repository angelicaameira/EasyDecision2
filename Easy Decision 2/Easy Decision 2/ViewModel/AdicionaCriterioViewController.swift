...
class AdicionaCriterioViewController: UIViewController, UITextFieldDelegate {
    ...
    override func viewDidLoad() {
        ...
        campoPeso.delegate = self
        botaoStepper.value = Double(campoPeso.text! as String) ?? 1
    }
    ...
    func salvarNovoCriterio(){
        if let decisao = decisao {
            if let idDecisao = decisao.id{
